// SPDX-License-Identifier: MIT
pragma solidity ^0.8.18;

import {Test, console} from "forge-std/Test.sol";
import {MoodNft} from "src/MoodNFT.sol";
import {DeployMoodNft} from "script/DeployMoodNft.s.sol";

contract MoodNftIntegrationTest is Test {
    MoodNft public moodNft;
    address public USER = makeAddr("USER");

    string public constant happySvgImageUri =
        "data:image/svg+xml;base64,";
    string public constant sadSvgImageUri =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_URI =
        "data:application/json;base64,";

    function setUp() public {
        DeployMoodNft deployer = new DeployMoodNft();
        moodNft = deployer.run();
    }

    function testMintingNftDefaultsToHappySvg() public {
        vm.prank(USER);

        moodNft.mintNft();

        MoodNft.Mood nftMood = moodNft.getNFTMood(0);
        assertEq(uint256(nftMood), uint256(MoodNft.Mood.HAPPY));
    }

    function testViewTokenUri() public {
        vm.prank(USER);

        moodNft.mintNft();
    }

    // Test: Flip Nft Mood to Sad
    function testFlipNftMoodToSad() public {
        vm.prank(USER);
        moodNft.mintNft(); // Here the mood is Happy

        vm.prank(USER);
        moodNft.flipMood(0); // Mood turns Sad

        MoodNft.Mood nftmood = moodNft.getNFTMood(0); // Here the mood should have turned to sad

        assertEq(uint256(nftmood), uint256(MoodNft.Mood.SAD));
        // assertEq(
        //     keccak256(abi.encodePacked(moodNft.tokenURI(0))),
        //     keccak256(abi.encodePacked(SAD_SVG_URI))
        // );
    }
}
